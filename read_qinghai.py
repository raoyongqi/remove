...
# 读取 CSV 数据
df = pd.read_csv('Annual_qinghai_ci.csv')
...
# 按照最后一年的覆盖度排序
labels = ['荒地', '耕地',  '草地']
values = [df['Barrenland'].iloc[-1], df['Cropland'].iloc[-1], df['Grassland'].iloc[-1]]
...
# 添加标题和标签
plt.title('青海省不同土地类型的植被覆盖度', fontsize=16)
plt.xlabel('年份', fontsize=12)
plt.ylabel('植被覆盖度 (Ci)', fontsize=12)

# 显示图表
plt.tight_layout()  # 确保图表不会被切割
plt.savefig('qinghai_ci.png')  # 保存图表
plt.show()  # 显示图表

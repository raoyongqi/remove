...
# 读取 CSV 数据
df = pd.read_csv('Annual_neimeng_ci.csv')
...
line_forest, = plt.plot(df['year'], df['Grassland'], label='草地', marker='o')
line_grassland, = plt.plot(df['year'], df['Woody savannas'], label='草地', marker='o')

# 按照最后一年的覆盖度排序
labels = ['荒地', '耕地', '草地', '木本稀树草原']
values = [df['Barrenland'].iloc[-1], df['Cropland'].iloc[-1], df['Grassland'].iloc[-1], df['Woody savannas'].iloc[-1]]
...
# 添加标题和标签
plt.title('内蒙自治区不同土地类型的植被覆盖度', fontsize=16)
plt.xlabel('年份', fontsize=12)
plt.ylabel('植被覆盖度 (Ci)', fontsize=12)

# 显示图表
plt.tight_layout()  # 确保图表不会被切割
plt.savefig('neimeng_ci.png')  # 保存图表
plt.show()  # 显示图表
